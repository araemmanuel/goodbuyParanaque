  ...
  function check_logged_in(method)
  {
	  ...
	  $.ajax({
				...
				success: function(data)
				{
					...
					if(method == "checkout")
					{
						...
						else
						{
							...
							{
								$('#avail_reward_card').append('<br/><div class="row clearfix"><div class="col-sm-6 col-lg-6"><div class="form-group"><input type="checkbox" id="md_checkbox_1" class="chk-col-green filled-in" checked /><label for="md_checkbox_1" class="font-12 col-black">I would like to avail a GB reward card.</label></div></div>');
							}
						}
					}
				}
			...
			$.ajax({
				...
				success: function(data)
				{
					...
					else
					{
						...
						$('#header_with_reward').append('<p>You currently have <b>'+data['reward_points']+' point(s)</b>.</p>')
					}
				}
			});
			$.ajax
		...
		else if(method=="verify_checkout")
		{
			...
			$('.common_element_view_banner').append('<h2>Order #'+order_no_page_verify+'</h2>');
		}
	}
				...
				success: function(data)
				{
					...
					else
					{
						data.forEach(function(element)
						{
							...
							{
								let n_selling_price = parseFloat(element['selling_price'] - parseFloat(element['selling_price'] * (parseFloat(element['discount_percent'] / 100))));
								price_of_order = parseFloat(price_of_order + parseFloat(parseFloat(element['selling_price'] - parseFloat(element['selling_price'] * (parseFloat(element['discount_percent'] / 100))))* element['quantity']));
							...
							{
								if(element['item_order_status'] != "Cancelled")
								temp = temp + '<button type="button" class="btn bg-green cancel_items_btn  waves-effect" data-toggle="modal" data-target="#'+element['sku']+'" data-target-modal="'+element['name']+'" data-product="'+element['name']+'">Cancel Item/s</button>';
							}
							temp = temp + '</td></tr>';
	  ...
	  $.ajax({
				...
				success: function(data)
				{
					...
					else
					{
						...
						$('#location_select_holder').append('<select class="form-control" name="location_select" id="location_select"><option vallue="Baclaran, Parañaque" >Baclaran, Parañaque</option><option value="Don Galo, Parañaque" >Don Galo, Parañaque</option><option value="La Huerta, Parañaque" >La Huerta, Parañaque</option><option value="San Dionisio, Parañaque" >San Dionisio, Parañaque</option><option value="San Isidro, Parañaque" >San Isidro, Parañaque</option><option value="City of Manila, Parañaque" >City of Manila, Parañaque</option><option value="Sto. Niño, Parañaque" >Sto. Niño, Parañaque</option><option >Tambo, Parañaque</option value="Vitalez, Parañaque" ><option>Vitalez, Parañaque</option><option value="BF Homes, Parañaque" >BF Homes, Parañaque</option><option value="Don Bosco, Parañaque" >Don Bosco, Parañaque</option><option value="Marcelo Green, Parañaque" >Marcelo Green, Parañaque</option><option value="Merville, Parañaque" >Merville, Parañaque</option><option value="Moonwalk, Parañaque" >Moonwalk, Parañaque</option><option value="San Antonio, Parañaque" >San Antonio, Parañaque</option><option value="San Martin de Porres, Parañaque" >San Martin de Porres, Parañaque</option><option value="Sun Valley, Parañaque" >Sun Valley, Parañaque</option></select><span class="small-smaller" id="location_select_label"></span>');
						$('#location_select').val(data['shipping_city']);
					...
					else
					{
						...
						data.forEach(function(element)
						{
							if(element['order_no'] != order_no)
							{
								...
								let sringed = 'Total Cost    : '+"&#8369;" + parseFloat(price_of_order).toFixed(2);
								$('#message_t_c').empty();
  ...
  {
	  var order_no_verify = window.location.href.substring(window.location.href.indexOf("verify_checkout") + 16);
			$('.common_element_view_banner').empty();
			$('.common_element_view_banner').append('<h2>Order #'+order_no_verify+'</h2>');
	  $.ajax({
		type: 'ajax',
		method: 'POST',
		url : base_url + 'customer/verify_order_function/'+order_no_verify,
		dataType : 'json',
		async : true,
		error: function(request, error)
		{
