	<section class="content">
        <div class="product_view_banner">
			<h2>Your Cart</h2>
			<ol class="breadcrumb breadcrumb-col-green">
				<li><a href="<?php echo base_url(); ?>" >Home</a></li>
				<li><a href="<?php echo base_url('customer/view_cart'); ?>">Cart</a></li>
			</ol>
		</div>
        <div class="bg-white">
            <div class="profile_view_holder">
                <div class="row">
                    <div class="col-lg-12 profile_view">
                        <div class="card profile_view_card">
                            <div class="row">
								<div class="col-lg-8 col-sm-12">
                                    <i class="material-icons col-green card-message">remove_shopping_cart</i>
                                    <div class="card-message-content">
                                        <br/>
                                        <h4>Cart Not Available.</h4>
                                        <p>Sorry but you can't access the page at the moment, it seems that you still don't have any product in your cart.</p>
                                    </div>
								</div>
							</div>
						</div>
					</div>
				</div>
			</div>
		</div>
	</section>
