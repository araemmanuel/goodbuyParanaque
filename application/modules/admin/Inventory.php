...
class Inventory extends My_Controller {
	...
	private function get_prod_variant_post($prod_id = null)
	{
		...
		if($prod_id)
		{
			...
			if($orig_subcat_id != $subcat_id) // if different subcategory
	...
	private function edit_prod_modal($prod_id)
	{
		...
		$this->update_skus($prod_id,  $this->input->post('modal-cat_name'), $this->input->post('modal-subcat_name'));
	...
	private function edit_prod($prod_id, $sku, $tblName = null)
	{
		...
		$prod_var_data = $this->get_prod_variant_post($prod_id);
		$prod_variant_edit = $this->mdl_product_variant->update($prod_id, $sku, $prod_var_data);		
		$sku = $prod_var_data['sku'];
	...
	*/
	...
	{
		$orig_subcat_id = $this->mdl_product->get_col_where('subcat_id', 'prod_id', $prod_id);
		$subcat_id =  $this->mdl_subcategory->get_subcat_id($cat_name, $subcat_name);
	...
	//Check if shirt, pants, and then footwear.
	...
	public function price_tags($date_from, $date_to, $tag_type)
	{	
           ...
           {  
               ...
               // $sub_array[] = '<img src="'.base_url().'upload/'.$row->image.'" class="img-thumbnail" width="50" height="35" />';  
				...
				else
				{
					...
					. '<button type="button" class="btn btn-xs bg-green waves-effect confirm"  data-title="Are you sure you want to deactivate this product?" data-url="'. base_url('admin/inventory/prod_del/'.$p->prod_id . '/' . $p->sku2 . '/products').'">Deactivate</button>'
