...
<?php
    $pdf = new Report(PDF_PAGE_ORIENTATION, PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false);
       ...
       //$pdf->SetKeywords('PDF, PDF, example, test, guide');
		...
		$pdf->setReportName('SUMMARY SOLD ITEMS');
...
//heredoc style no white spaces after eod; and must be aligned with php tag
        ...
        ob_end_clean();
        if($is_send_email == 1)
			$pdf->Output(FCPATH.'reports/summary_sold_'.date('M-d-Y', strtotime($date_from)).'-'.date('M-d-Y', strtotime($date_to)).'.pdf', 'F');
		else
			$pdf->Output('summary_sold.pdf', 'I');
