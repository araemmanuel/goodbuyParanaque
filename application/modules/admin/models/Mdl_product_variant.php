...
class Mdl_Product_Variant extends My_Model{
	...
	function get_options_by_sku($sku)
	{
			$sql = "SELECT DISTINCT PO.sku, O.opt_name, O.opt_grp_id FROM `tblproduct_option` PO
					LEFT JOIN tbloption O ON O.opt_id = PO.option_id 
					WHERE PO.sku = ?;";
		$q = $this->db->query($sql, array($sku));
		return $q->result();
	}
	function get_skus_by($type, $id)
	{
		if($type == 'category')
		{
			$sql = "SELECT PV.sku FROM `tblproduct_variant` PV
	...
	function get_min_qty_pvs($reorder_point)
	{
		$sql = "SELECT P.prod_id, PV.sku, P.name, PV.quantity, PV.selling_price,
				...
				(SELECT GROUP_CONCAT(DISTINCT O.opt_name ORDER BY O.opt_grp_id ASC) FROM tblproduct_option PO 
							LEFT JOIN tbloption O ON O.opt_id = PO.option_id 
							WHERE PO.sku = PV.sku ORDER BY O.opt_name) as options, PV.date_zeroed
	...
	function get_tag_info_by_date_sku($date_from, $date_to, $sku)
	{
		$sql = "SELECT PV.sku, P.name, PV.selling_price,
				(SELECT SUM(SHS.qty)  FROM tblstock_history SHS 
				 WHERE SHS.sku = PV.sku AND SHS.date_added >= ? AND SHS.date_added <= ? GROUP BY SHS.sku) as quantity, 
				...
				(SELECT IF(COUNT(I.img_file_path) = 0 ,'None', I.img_file_path)  FROM tblimage I
				 ...
				 LEFT JOIN tblproduct P ON P.prod_id = PV.prod_id 
				 LEFT JOIN tblstock_history SH ON SH.prod_id = PV.prod_id AND SH.sku = PV.sku 
				 WHERE SH.date_added >= ? AND SH.date_added <= ? AND SH.sku = ? AND PV.is_active = 1 GROUP BY SH.sku";
		$q = $this->db->query($sql, array($date_from, $date_to, $date_from, $date_to, $sku));
		return $q->result();	
	}	
	...
	function get_pv($id, $sku, $active = 1){
		$sql = "SELECT P.prod_id, PV.sku, PV.is_customer_viewable, 
				...
				(SELECT S.name AS sup_name FROM tblsupplier S
						WHERE S.id = P.supplier_id) as sup_name,

				P.supplier_id, P.name, P.brand, P.description, PV.selling_price, PV.purchase_price, PV.quantity, SC.subcat_name, C.cat_name, SC.subcat_id,
	...
	function get_by_prod($prod_id, $type=null){
		$sql = "SELECT P.prod_id, PV.sku, P.name, P.brand, PV.quantity, PV.purchase_price, PV.selling_price, PV.is_customer_viewable,
					...
					(SELECT date_added FROM `tblstock_history` WHERE sh_id = (SELECT MAX(sh_id) FROM tblstock_history WHERE prod_id = P.prod_id)) AS last_updated
					FROM tblproduct_variant PV 
	...
	function dt_pv_qry()
	{
		if($_POST["length"] != -1)  
        {  
			$sql = "SELECT P.prod_id, C.cat_name, SC.subcat_name, PV.sku, P.name, P.brand, PV.selling_price, PV.purchase_price, PV.quantity,
				(SELECT GROUP_CONCAT(DISTINCT O.opt_name ORDER BY O.opt_grp_id ASC) FROM tblproduct_option PO
						LEFT JOIN tbloption O ON O.opt_id = PO.option_id WHERE PO.prod_id = P.prod_id AND PO.sku =  PV.sku ORDER BY O.opt_name) as options,
			    (SELECT IF(COUNT(I.img_file_path) = 0 ,'None', I.img_file_path)  FROM tblimage I
										NATURAL JOIN tblproduct_variant PVS 
				...
				LEFT JOIN tblsubcategory SC ON SC.subcat_id = P.subcat_id
				LEFT JOIN tblcategory C ON C.cat_id = SC.cat_id
				WHERE PV.is_active = 1 AND PV.quantity > 0 AND CONCAT(PV.sku, P.name, P.brand) LIKE ? ORDER BY P.prod_id DESC LIMIT ?, ?;";
		...
		else
		{
			$sql = "SELECT P.prod_id, C.cat_name, SC.subcat_name, PV.sku, P.name, P.brand, PV.selling_price, PV.purchase_price, PV.quantity,
				...
				LEFT JOIN tblsubcategory SC ON SC.subcat_id = P.subcat_id
				LEFT JOIN tblcategory C ON C.cat_id = SC.cat_id
				WHERE PV.is_active = 1 AND PV.quantity > 0 AND CONCAT(PV.sku, P.name, P.brand) LIKE ?;";
   ...
   {  
	...
	{  
		$sql = "SELECT P.prod_id, C.cat_name, SC.subcat_name, PV.sku, P.name, P.brand, PV.selling_price, PV.purchase_price, PV.quantity,
				...
				LEFT JOIN tblsubcategory SC ON SC.subcat_id = P.subcat_id
				LEFT JOIN tblcategory C ON C.cat_id = SC.cat_id
				WHERE PV.is_active = 1 AND CONCAT(PV.sku, P.name, P.brand) LIKE ?;";
		$q = $this->db->query($sql, array('%'.$_POST["search"]["value"].'%'));
		return $q->num_rows();
    }     
