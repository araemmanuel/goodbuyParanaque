...
class Mdl_Product extends My_Model{
	...
	{		
		$sql = "SELECT P.prod_id, P.name, P.brand, P.description, SC.subcat_name, C.cat_name, C.cat_id, S.name sup_name, S.id sup_id FROM tblproduct P
					LEFT JOIN tblsubcategory SC ON SC.subcat_id = P.subcat_id
					LEFT JOIN tblcategory C	ON C.cat_id = SC.cat_id
					LEFT JOIN tblsupplier S ON S.id = P.supplier_id
					WHERE P.prod_id = ?;";
	...
	function get_pcode_num($cat_id, $subcat_id, $exempted_sku = null)
	{
		if($exempted_sku)
		{
			$sql = "SELECT  IF(CHAR_LENGTH(C.cat_code) = 1, MAX(SUBSTR(PV.`sku`, 5) + 0), MAX(SUBSTR(PV.`sku`, 6) + 0)) as m FROM `tblproduct_variant` PV
	...
	function dt_product_qry()
	{
        ...
        {  
			$sql = "SELECT P.prod_id,PV.sku sku2, P.name, P.description, P.brand, SC.subcat_name, C.cat_name, PV.is_customer_viewable, PV.quantity, PV.selling_price, PV.purchase_price,
								...
								(SELECT  IF( COUNT(sku) = 1, sku, 'has_multiple_var') FROM tblproduct_variant WHERE prod_id = P.prod_id AND is_active = 1) AS prod_var, 
                                (SELECT GROUP_CONCAT(DISTINCT selling_price) FROM `tblproduct_variant` WHERE prod_id = P.prod_id) AS selling_price,
								...
								(SELECT SUM(quantity) FROM `tblproduct_variant` WHERE prod_id = P.prod_id) AS stock
								FROM `tblproduct` P 
								NATURAL JOIN tblsubcategory SC
								NATURAL JOIN tblcategory C	
								NATURAL JOIN tblproduct_variant PV
...
/*
SELECT P.prod_id,PV.sku sku2, P.name, P.description, P.brand, SC.subcat_name, C.cat_name, PV.is_customer_viewable, PV.quantity, PV.selling_price, PV.purchase_price,
								IF(CHAR_LENGTH(C.cat_code) = 1, SUBSTR(PV.`sku`, 1, 4), SUBSTR(PV.`sku`, 1, 5)) AS sku,		
								(SELECT  IF( COUNT(sku) = 1, sku, 'has_multiple_var') FROM tblproduct_variant WHERE prod_id = P.prod_id AND is_active = 1) AS prod_var, 
                                (SELECT GROUP_CONCAT(DISTINCT selling_price) FROM `tblproduct_variant` WHERE prod_id = P.prod_id) AS selling_price,
                                (SELECT date_added FROM `tblstock_history` WHERE sh_id = (SELECT MAX(sh_id) FROM tblstock_history WHERE prod_id = P.prod_id)) AS last_updated,
								...
								NATURAL JOIN tblcategory C	
								NATURAL JOIN tblproduct_variant PV
                                WHERE PV.is_active = 1 AND CONCAT(P.name, SC.subcat_name, C.cat_name, sku) LIKE '%%' GROUP by P.prod_id ORDER BY P.prod_id DESC LIMIT 0, 10
