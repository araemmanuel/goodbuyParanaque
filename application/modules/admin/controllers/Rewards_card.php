...
class Rewards_Card extends My_Controller {
	...
	{
		$this->form_validation->set_rules('firstname', 'First Name', 'trim|required|callback_letter_space|callback_member_check');
		$this->form_validation->set_rules('lastname', 'Last Name', 'trim|required|callback_letter_space');
		$this->form_validation->set_rules('middlename', 'Middle Name', 'trim|callback_letter_space');
		$this->form_validation->set_rules('dob', 'Date of Birth', 'trim|callback_age_check');
		$this->form_validation->set_rules('gender', 'Gender', 'trim');
		$this->form_validation->set_rules('email', 'Email', 'trim|valid_email|is_unique[tbluser.email]');
		$this->form_validation->set_rules('address', 'Shipping Address', 'trim');
		$this->form_validation->set_rules('city', 'City', 'trim');
		$this->form_validation->set_rules('zipcode', 'Shipping Zipcode', 'trim|integer');
	...
	{
		$middlename = null;
		$fname_uid = $this->mdl_user->get_col_where('id', 'firstname', 'Laurena sigrid');
		$lname_uid = $this->mdl_user->get_col_where('id', 'lastname',  'Garcia');
		$dob_uid = $this->mdl_customer->get_col_where('user_id', 'DOB', '2018-06-16');	
		if($middlename)
		{	
			$mname_uid = $this->mdl_user->get_col_where('id', 'middlename', $middlename);			
			$condition = (($fname_uid == $lname_uid) && ($fname_uid == $mname_uid)  && ($lname_uid == $dob_uid));
		}
		else
		{
			$condition = (($fname_uid == $lname_uid) && ($lname_uid == $dob_uid));
	...
	public function member_check()
	{
		...
		if ($this->mdl_user->exists2($fname, $mname, $lname, $dob))
		{
			$this->form_validation->set_message(__FUNCTION__, 'Member already exists.');
			return FALSE;
		}
		else
		{
	...
	private function process_add()
	{
		...
		$this->log_data("Card Holder Add", $card_data['membership_id'] ." member was added.");	
	}
	...
	private function process_edit($card_no)
	{
		...
		if($this->mdl_user->_update('id', $user_id, $cust2_data))
			$this->session->set_flashdata('alert_msg','Card Holder Edited Successfully!');
		$this->log_data("Card Holder Edit", $card_data['membership_id'] ." member was edited.");	
	...
	public function del_card_holder($id)
	{
		...
		if($this->mdl_rewards_card->can_delete($id) && $this->mdl_rewards_card->can_delete2($user_id))
		{	
			$mem_id = $this->mdl_rewards_card->get_col_where('membership_id', 'card_no', $id);
			$this->log_data("Card Holder Delete", $mem_id ." member was deleted.");	
			if($this->mdl_rewards_card->_delete('user_id', $user_id)  && $this->mdl_user->_delete('id', $user_id))
	...
	public function batch_print()
	{
		...
		$card_data['card_qr_path'] = $this->card_qr_path;
		$this->log_data("Reward Cards Batch Print", "Generated reward cards for batch print.");		
	...
	public function cust_app_form()
	{
		...
		$this->log_data("Reward Cards App. Form Print", "Generated ".$data['qty']. " rewards card application forms.");	
		$this->load->view('vw_admin_cust_app_form', $data);	
		
	}
	public function card_details($card_no)
	{
		$mem_id = $this->mdl_rewards_card->get_col_where('membership_id', 'card_no', $card_no);		
		if($mem_id)
		$this->log_data("Card Details View", "Viewed card details for member ".$mem_id.".");	
	...
	public function print_reward_card($card_no, $membership_id)
	{
		...
		$data['card_qr_path'] = $this->card_qr_path;
		$data['membership_id'] = $membership_id;
		$data['is_batch_print'] = false;
		$this->load->view('vw_admin_print_card', $data);		
		$this->log_data("Reward Cards Single Print", "Generated rewards card for member ".$membership_id.".");		
	}
	//Other functions
	...
	{  
           ...
           {  
                $sub_array = array();   
				$sub_array[] = '<input type="checkbox" id="chk-card-body" class="chk-col-green" name="chk-reward-card[]" value="'.$c->card_no.'"/><label for="chk-card-body"></label>';
				$sub_array[] = $c->card_no;
				...
				$sub_array[] = $c->status;  
				$sub_array[] = '<a href="'.base_url('admin/rewards_card/card_details/'.$c->card_no).'" class="btn btn-xs bg-default waves-effect" style="background-color:#ddd;color:#555!important;">View</a>
	...
	function get_user_data()
	{
		...
		if(!empty($this->input->post('middlename')))
			$user_data['middlename'] = ucwords($this->input->post('middlename'));
		else
			$user_data['middlename'] = null;	
