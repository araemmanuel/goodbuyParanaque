...
class Transfer extends My_Controller {
	...
	{  
           ...
           {  
				...
				$sub_array[] = $p->prod_id;
				$sub_array[] = '<input type="checkbox" class="chk-col-green" name="items" id = "chk-'.$p->sku.'" data-prod-code = "'.$p->sku.'" data-qty="'.$p->quantity.'" data-name="'.$p->name.'"  data-options="'.$p->options.'"/><label for="chk-'.$p->sku.'"></label>';
				$sub_array[] = $p->sku;
				$sub_array[] = $p->name;
				$sub_array[] = $p->options;
				$sub_array[] = $p->brand;
