...
class Mdl_Invoice extends My_Model{
	...
	{
		$sql = "SELECT IL.invoice_no, P.name AS prod_name, IL.amt_paid, IL.discount, IL.selling_price, IL.qty AS quantity,  IL.is_void, 
				(SELECT GROUP_CONCAT(DISTINCT O.opt_name ORDER BY O.opt_grp_id ASC)
		 FROM tblproduct_option PO LEFT JOIN tbloption O ON O.opt_id = PO.option_id WHERE PO.sku = IL.sku)  AS options 
					FROM `tblinvoice` I
