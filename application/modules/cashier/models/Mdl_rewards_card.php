...
class Mdl_Rewards_Card extends My_Model{
	...
	{
		$sql = "SELECT membership_id FROM `tblreward_card` RC WHERE DATE(?) > RC.expiration_date AND membership_id = ?";
		$q = $this->db->query($sql, array($membership_id, date('Y-m-d')));
		if ($q->num_rows() == 1){
           return true;
        }else{ 
           return false;
	...
	function get_card_holders()
	{
		$sql = "SELECT RC.card_no, RC.membership_id, CONCAT(U.firstname, ' ', U.lastname) AS name, RC.expiration_date, RC.reward_points, 
				IF(DATE(?) > RC.expiration_date, 'EXPIRED', 'VALID') AS status
	...
	function get_card_details($card_no)
	{
		$sql = "SELECT RC.card_no, RC.membership_id, CONCAT(U.firstname, ' ', U.lastname) AS name, 
				...
				WHERE RC.card_no = ?";
		$q = $this->db->query($sql, array(date('Y-m-d'), (double)$card_no));
		return $q->result();
	}
	...
	function get_membership_id($date_registered)
	{
		...
		$sql = "SELECT MAX(CAST(digits(SUBSTR(`membership_id`, 5, 10)) AS INT)) AS m FROM `tblreward_card` 
        ...
        else
		{
			...
			return date('Y', strtotime($date_registered)) . '-'. $id  . '-TG-0';
		}	
	}
